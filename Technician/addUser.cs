...
namespace Mngs_Client_Support.Technician
{
    ...
    public class addUser : Activity, IOnSuccessListener, IOnFailureListener
    {
        ...
        private bool validfound()
        {
            ...
            try
            {
                if (preferdName.Text.Trim().IndexOf(" ") > 0 || preferdName.Text.Trim().IndexOf("  ") > 0 || preferdName.Text.Trim().ToLower()=="none")
                {
                    Result = false;
                    preferdName.Error = "spaces are not allowed within your text and name cannot be set as 'none' ";
                }
                ...
                if (preferdName.Text.Trim().IndexOf(" ") == -1 && preferdName.Text.Trim().IndexOf("  ") == -1 &&
                    ...
                    emailTxt.Text.Trim().IndexOf(" ") == -1 && emailTxt.Text.Trim().IndexOf("  ") == -1 && passTxt.Text.Trim().IndexOf("  ") == -1)
                {
                    ...
                    {
                        Result = false;
                        passTxt.Error = "password must be 6 to more characters. without spaces in the beginning and the end.";
                    }
                }
        ...
        void submitMsql()
        {
            try
            {
                ...
                if (status == NetworkAccess.Internet)
                {
                    ...
                    collection.Add("username", preferdName.Text.Trim().ToLower());
                    collection.Add("phone", phone.Text.Trim());
                    collection.Add("AccountType", "Technician");
                    collection.Add("email", emailTxt.Text.Trim());
        ...
        public void OnSuccess(Java.Lang.Object result)
        {
            ...
            data.Put("pass", passTxt.Text.Trim());
            data.Put("phone", phone.Text.Trim());
            data.Put("jobID", "none");
            data.Put("tasked", "no");
            data.Put("AccountType", "Technician");
            data.Put("email", emailTxt.Text.Trim());
            DatabaseReference databaseRef = firebase_Helper.GetDatabase().GetReference("Users").Child(auth.CurrentUser.Uid);
